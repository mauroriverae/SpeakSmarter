            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex justify-between" v-if="$page.props.user.permissions.includes('create lessons')">
                        <Link :href="route('lessons.create')" class="text-white bg-indigo-500 hover:bg-indigo-700 py-2 px-4 rounded">Create lesson</Link>
                    </div>
                    <div class="mt-4">
                            ...
                            <li class="flex justify-between gap-x-6 py-5" v-for="lesson in lessons.data">
                                ...
                                <div class="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
                                    <p class="text-md leading-6 text-gray-900">
                                        ...
                                        <Link class="py-2 px-4 text-red-600" v-if="$page.props.user.permissions.includes('delete lessons')" @click="deleteLesson(lesson.id)">Delete</Link>
                                    </p>
                                </div>
                            </li>
                        </ul>
                    ...
                    <div class="flex justify-between mt-2">
                        <Link v-if="lessons.current_page > 1" :href="lessons.prev_page_url" class="py-2 px-4 rounded">PREV</Link>
                        <div v-else></div>
                        <Link v-if="lessons.current_page < lessons.last_page" :href="lessons.next_page_url" class="py-2 px-4 rounded">NEXT</Link>
                    </div>
                </div>
            </div>
        </div>
